...
int Kruskal(Edge *E, int Esize) {
    ...
    while(Esize > 0) {                   //部分木に取り込んでいない辺の数
        minimum = delete_root(E, &Esize); // heapから最小コストの辺を取り出す
            ...
            // printf("%d %d %d %d\n", minimum.v_1, minimum.v_2, S1, S2);
        if(S1 != S2) {         //もし、２つの頂点が同じ部分木に属していないのであれば
            Union_tree(S1, S2); //その２頂点を結ぶ辺を採用し、部分木同士を結合する
            printf("[Accept] Selected Edge: (%c,%c), cost=%d\n", (char)minimum.v_1 + 65, (char)minimum.v_2 + 65,
                   minimum.cost);  //採用した辺を表示(出力の都合、添字+1)
            costs += minimum.cost; //採用した辺のコストを計上する
        } else {
            printf("[Deny] Selected Edge: (%c,%c), cost=%d\n", (char)minimum.v_1 + 65,(char) minimum.v_2 + 65,
                   minimum.cost);  //採用していない辺を表示(出力の都合、添字+1)
        }
    }
    return costs;
}
